...
export default async function ExplorePage({ searchParams }: { searchParams: Promise<{ destination?: string; checkIn?: string; checkOut?: string; guests?: string }> }) {
  ...
  return (
    <Page wide>
      <PageHeader eyebrow="Quintana Roo" title="¿Dónde quieres vivir este tiempo?" description="Encuentra estancia por fechas. All Living conecta tiempo disponible real con tu viaje." />
      ...
      {hasDates ? <Section title={inventory.length ? `${inventory.length} opciones disponibles` : "Sin tiempo disponible para esas fechas"}>
        ...
        <Section title="Empezamos en el Caribe"><ul className="grid grid-cols-3 gap-3 md:gap-5">{DESTINATIONS.map(d=><li key={d.slug}><PhotoTile href={`/explore/${d.slug}`} src={d.photo} alt={d.name} title={d.name} subtitle={d.tags}/></li>)}</ul></Section>
        <Section title="Propiedades para vivir"><ul className="grid grid-cols-2 gap-4 md:grid-cols-4">{properties.filter(p=>["Cancún","Playa del Carmen","Tulum"].includes(p.destination)).map(p=><li key={p.id}><PhotoTile href={`/properties/${p.id}`} src={p.cover?.url??null} alt={p.cover?.alt??p.name} title={p.name} subtitle={`${p.destination}${p.bedrooms?` · ${p.bedrooms} rec.`:""}`} badge={p.isDemo?<Chip>Demo</Chip>:undefined}/></li>)}</ul></Section>
      </>}
    </Page>
  );
}
