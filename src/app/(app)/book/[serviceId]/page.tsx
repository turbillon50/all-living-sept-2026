...
export default async function BookPage({ params, searchParams }: { params: Promise<{ serviceId: string }>; searchParams: Promise<{ stay?: string }> }) {
  ...
  return (
    <Page>
      <TopBar back={`/providers/${svc.provider.slug}`} title="Confirmar reserva" />
      <h1 className="pt-4 text-[28px] leading-[1.08]">Confirmar reserva</h1>
      <div className="mt-5 flex items-center gap-4 rounded-[var(--radius-card)] bg-surface hairline p-3">
        {svc.service.coverUrl ? <Photo src={svc.service.coverUrl} alt="" className="w-20 shrink-0" ratio="1/1" sizes="80px" /> : null}
        <div className="min-w-0"><p className="text-[15px] font-medium truncate">{svc.service.name}</p><p className="text-[13px] text-text-2 truncate">{svc.provider.businessName} · All Living Verified</p></div>
      </div>
      <BookingForm
